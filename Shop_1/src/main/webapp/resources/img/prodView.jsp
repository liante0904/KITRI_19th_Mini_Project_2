...
function down(a){
	if(document.f.cnt2.value > 1){
		document.f.cnt2.value--;
		document.f.totalprice.value = a * document.f.cnt2.value;
	}
}
function up(a){
...
<body>
	...
	<table border='1px'>
		...
		<tr>
			<td colspan="2"><fmt:formatNumber value="${product.prod_price}" pattern="#,###.##" />원</td>
		</tr>
		<tr>
			<td colspan="2">수량:
				<div class="plusminus">
					<form name="f">
					<input type="text" id="cnt" class="text" title="수량설정"
				value="1" name="cnt2" size="3">
					<button type="button" class="defbtn_minus"  onclick="down(${product.prod_price})">수량감소</button>
					<button type="button" class="defbtn_plus" onclick="up(${product.prod_price})">수량증가</button>
					
				</div>
			</td>
		</tr>
		<tr>
			<td colspan="2">총 가격: <span id="totalPrice" ><input type="text" name="totalprice" value= "<fmt:formatNumber value="${product.prod_price}" pattern="#,###.##" />" readonly></span>
			</td>
		</tr>
		</form>
		<tr>
...
<tr><th align="center">평가</th><th>내용</th><th>작성자</th></tr>
...
<tr><td align="center">${a}</td><td>${list.cmt_comment}</td><td>${list.id}</td></tr>
...
<form action="${pageContext.request.contextPath}/writeCmt.do" method="POST" name="f">
...
<input type="text" name="cmt_comment" style="width:1000px;" />

<input type="submit" name="searchh" value="작성">
